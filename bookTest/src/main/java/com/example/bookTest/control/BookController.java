...
public class BookController {
	...
	public ModelAndView home() {
		...
		List<BookinDto> list= bookService.selectAll();
	...
	@PostMapping("/enroll")
	public String write( @ModelAttribute BookinDto bookinDto) {
		// form태그안에 입력한 값은 BookDto클래스의 객체에 저장 되어있다.
		// 데이터베이스에 저장 하려면 BookDto객체를 DAO에 넘겨서 저장하면된다.
		// 컨트롤 -> service -> DAO
		bookService.bookSave(bookinDto);
	...
	public ModelAndView view(@RequestParam(value="id", 
		...
		BookinDto data=bookService.getBook(id);
		
		if(data==null) data=new BookinDto(); //getBook메서드의 반환값으로 null 저장 된다면
...
//		return "<script> alert("삭제되었습니다")
		...
		public String bookUpdate( @ModelAttribute BookinDto bookinDto, 
				@RequestParam("id") int id) {
			bookinDto.setBookId(id);
			bookService.update(bookinDto);
